    ...
    if (viewboxAttribute !== undefined) {
        ...
        if (widthAttribute !== undefined) {
            ...
            if (isValidUnit(widthUnit) && width !== undefined) {
                const widthInProjectUnits = width * scaleFactorForUnitConversion(widthUnit, options.units);
...
export const pathOrShapeToSVGString = (item, options) => {
    ...
    if (!stroke) {
        ...
        const customAlignment = !stroke.hairline &&
        ...
        if (customAlignment && options?.useSVGPathClipping) {
            ...
            result += `<path d="${d}" clip-path="url(#${clipId})" fill="none" stroke="${stroke.color.toCSSString()}" stroke-width="${stroke.width * 2}" ${strokeAttrs}/>`;
            return result;
        }
        if (customAlignment) {
