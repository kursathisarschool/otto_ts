...
export class Color {
    ...
    static fromCSSString(cssString) {
        ...
        if (fromKeyword) {
            return new Color(fromKeyword[0] / 255, fromKeyword[1] / 255, fromKeyword[2] / 255, fromKeyword[3]);
        }
        // hex
        if (cssString[0] === '#') {
            if (cssString.length === 4) {
                ...
                return new Color(fromHex1(cssString[1]), fromHex1(cssString[2]), fromHex1(cssString[3]), fromHex1(cssString[4]));
            }
            else if (cssString.length === 7) {
                return new Color(fromHex2(cssString.slice(1, 3)), fromHex2(cssString.slice(3, 5)), fromHex2(cssString.slice(5, 7)));
            }
            else if (cssString.length === 9) {
                return new Color(fromHex2(cssString.slice(1, 3)), fromHex2(cssString.slice(3, 5)), fromHex2(cssString.slice(5, 7)), fromHex2(cssString.slice(7, 9)));
            }
        }
        // rgb
        ...
        // rgba
        const rgba = cssString.match(/^rgba\s*\(\s*(\d+%?)[\s,]+(\d+%?)[\s,]+(\d+%?)[\s,]+(\d+\.?\d*%?)\s*\)$/);
        if (rgba) {
            return new Color(from255String(rgba[1]), from255String(rgba[2]), from255String(rgba[3]), fromAlphaString(rgba[4]));
        }
