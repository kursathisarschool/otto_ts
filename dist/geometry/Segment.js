...
import { bernsteinBezierFormForClosestPointOnCubic, cubicByTrimmingCubic, cubicsBySplittingCubicAtTime, findRoots, pointOnCubicAtTime, positionAndTimeAtClosestPointOnCubic, } from './bezier.js';
...
export const lineCubicIntersections = ([a1, a2], [b1, b2, b3, b4]) => {
    ...
    const roots = solveCubic(-d0 + 3 * d1 - 3 * d2 + d3, 3 * d0 - 6 * d1 + 3 * d2, -3 * d0 + 3 * d1, d0);
    const results = [];
...
const cubicsAlmostEqual = ([a1, a2, a3, a4], [b1, b2, b3, b4], tolerance) => {
    return ((a1.distance(b1) <= tolerance &&
...
export const cubicCubicIntersections = (cubic1, cubic2) => {
    ...
    if (cubicsEqual(cubic1, cubic2) || cubicsAlmostEqual(cubic1, cubic2, tolerance))
        return [];
    if (!cubicBoundingBoxesOverlap(cubic1, cubic2))
        return [];
    if (cubicsOverlap(cubic1, cubic2, tolerance))
        return [];
    ...
    for (let i = 0; i < maxIterations; i++) {
        ...
        for (const { time1, cubic1, time2, cubic2 } of candidates) {
            const keepExploring = i < boundingBoxIterations
            ...
            if (keepExploring) {
                ...
                nextCandidates.push({ time1, cubic1: cubic1a, time2, cubic2: cubic2a }, { time1, cubic1: cubic1a, time2: time2 + nextTimeLength, cubic2: cubic2b }, { time1: time1 + nextTimeLength, cubic1: cubic1b, time2, cubic2: cubic2a }, {
...
export const primitivePrimitiveIntersections = (p1, p2) => {
    if (p1.length === 2) {
        // p1 is Line
        if (p2.length === 2)
            return lineLineIntersections(p1, p2);
        else
            return lineCubicIntersections(p1, p2);
    }
    else {
        // p1 is Cubic
        if (p2.length === 2)
            return cubicLineIntersections(p1, p2);
        else
            return cubicCubicIntersections(p1, p2);
    }
};
// =============================================================================
// Re-exports from bezier.js
// =============================================================================
export { pointOnCubicAtTime, cubicsBySplittingCubicAtTime, cubicByTrimmingCubic, positionAndTimeAtClosestPointOnCubic, };
