...
export class Transform {
    ...
    constructor(position: Vec, rotation: number, scale: Vec | number, skew: number, origin: Vec | null = null) {
    ...
    equalsWithinRelativeEpsilon(transform: Transform, epsilon: number = DEFAULT_EPSILON): boolean {
        return (
...
export class AffineMatrix {
    ...
    preTranslate(v: Vec): AffineMatrix {
        this.tx += v.x;
        this.ty += v.y;
        return this;
    }
    ...
    /** Apply uniform scale. */
    scaleScalar(s: number): AffineMatrix {
        this.a *= s;
        this.b *= s;
        this.c *= s;
        this.d *= s;
    ...
    skew(angle: number): AffineMatrix {
    ...
    origin(origin: Vec): AffineMatrix {
        const ox = -origin.x;
        const oy = -origin.y;
    ...
    changeBasis(changeOfBasisMatrix: AffineMatrix, inverseChangeOfBasisMatrix?: AffineMatrix): AffineMatrix {
    ...
    ensureMinimumBasisLength(length: number): AffineMatrix {
        const { a, b, c, d } = this;
    ...
    isOrthogonal(tolerance: number = DEFAULT_TOLERANCE): boolean {
        const { a, b, c, d } = this;
    ...
    isUniformScale(tolerance: number = DEFAULT_TOLERANCE): boolean {
        const { a, b, c, d } = this;
    ...
    toExpressionCode(minimumFractionDigits?: number, maximumFractionDigits?: number): string {
        const { a, b, c, d, tx, ty } = this;
        const expr = (x: number) => expressionCodeForNumber(x, minimumFractionDigits, maximumFractionDigits);
    ...
    static inverse(matrix: AffineMatrix): AffineMatrix {
        return matrix.clone().invert();
    }

    /** Return product of two matrices. */
    static mul(a: AffineMatrix, b: AffineMatrix): AffineMatrix {
        return a.clone().mul(b);
    }

    /** Create matrix from transform arguments. */
    static fromTransform({ position, rotation, scale, skew, origin }: TransformArgs): AffineMatrix {
    ...
    static fromCenterScale(center: Vec, scale: Vec): AffineMatrix {
        const { x, y } = center;
    ...
    static fromCenterAndRotationPoints(center: Vec, p1: Vec, p2: Vec): AffineMatrix {
        const { x, y } = center;
    ...
    static fromCenterAndQuantizedRotationPoints(center: Vec, p1: Vec, p2: Vec, incrementDegrees: number): AffineMatrix {
        const { x, y } = center;
        const t1 = Math.atan2(p1.y - y, p1.x - x);
        const t2 = Math.atan2(p2.y - y, p2.x - x);
        const radians =
    ...
    static fromCenterAndUniformScalePoints(center: Vec, p1: Vec, p2: Vec): AffineMatrix {
        const { x, y } = center;
    ...
    static fromCenterAndNonUniformScalePoints(center: Vec, p1: Vec, p2: Vec): AffineMatrix {
        const { x, y } = center;
        const dx = p1.x - x;
        const dy = p1.y - y;
    ...
    static fromCenterAndYAxis(center: Vec, yAxis: Vec): AffineMatrix {
        return new AffineMatrix(yAxis.y, -yAxis.x, yAxis.x, yAxis.y, center.x, center.y);
    }

    /** Parse SVG transform attribute string. */
    static fromSVGTransformString(transformString: string): AffineMatrix {
